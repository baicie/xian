...
test('registers a workspace, creates a task, and opens a document editor', async ({ page }) => {
  ...
  await page.getByLabel('输入任务标题，按回车创建').fill('登录 API 契约')
  await page.getByRole('button', { name: '创建', exact: true }).click()
  await expect(page.getByText('登录 API 契约', { exact: true })).toBeVisible()
  await page.setViewportSize({ width: 390, height: 844 })
  const blockedTaskCard = page.locator('.task-card').filter({ hasText: '登录服务异常' })
  await blockedTaskCard.click()
  const dependencySheet = page.getByRole('dialog', { name: '任务详情' })
  await expect(dependencySheet.getByText('暂无任务依赖')).toBeVisible()
  await dependencySheet.getByRole('button', { name: '添加前置任务' }).click()
  const dependencyPicker = page.getByRole('dialog', { name: '添加前置任务' })
  const dependencyPickerBox = await dependencyPicker.boundingBox()
  expect(dependencyPickerBox).not.toBeNull()
  expect(dependencyPickerBox!.x).toBeGreaterThanOrEqual(0)
  expect(dependencyPickerBox!.x + dependencyPickerBox!.width).toBeLessThanOrEqual(390)
  await dependencyPicker.getByLabel('搜索任务').fill('登录 API')
  await dependencyPicker.getByRole('button', { name: /登录 API 契约/ }).click()
  await expect(dependencySheet.getByText('1 项阻塞', { exact: true })).toBeVisible()
  await expect(dependencySheet.getByText('登录 API 契约', { exact: true })).toBeVisible()
  const blockerDependencyRow = dependencySheet
    .locator('.task-dependency-row')
    .filter({ hasText: '登录 API 契约' })
  await expect(blockerDependencyRow.getByText('待处理', { exact: true })).toBeVisible()
  await expect(blockerDependencyRow.getByText('阻塞', { exact: true })).not.toBeVisible()
  await page.keyboard.press('Escape')
  await expect(blockedTaskCard.getByTitle('1 项未完成前置任务')).toBeVisible()
  await page.setViewportSize({ width: 1280, height: 720 })
  ...
  await expect(page.locator('.iteration-task-table').getByText('1 项阻塞')).toBeVisible()
  await expect(page.locator('.iteration-health').getByText('未关闭 Bug')).toBeVisible()
  await expect(page.locator('.iteration-health').getByText('阻塞任务')).toBeVisible()

...
function ContextMenu({state,labels,onNewDocument,onNewFolder,onRename,onDuplicate,onMove,onRemove}:{state:NonNullable<ContextState>;labels:Record<string,string>;onNewDocument:(folderId:string|null)=>void;onNewFolder:(parentId:string|null)=>void;onRename:(target:DocumentFolder|DocumentSummary)=>void;onDuplicate:(target:DocumentSummary)=>void;onMove:(target:DocumentSummary)=>void;onRemove:(target:DocumentFolder|DocumentSummary)=>void}) {const target=state.target,folderId=target.type==='folder'?target.folder.id:null;return createPortal(<div className="document-context-menu" style={{left:state.x,top:state.y}} role="menu" onClick={event=>event.stopPropagation()}>{target.type!=='document'?<><button onClick={()=>onNewDocument(folderId)}><FilePlus2/>{labels.newDocument}</button><button onClick={()=>onNewFolder(folderId)}><FolderPlus/>{labels.newFolder}</button></>:null}{target.type!=='root'?<><hr/><button onClick={()=>onRename(target.type==='folder'?target.folder:target.document)}><Pencil/>{labels.rename}</button>{target.type==='document'?<><button onClick={()=>onDuplicate(target.document)}><Copy/>{labels.duplicate}</button><button onClick={()=>onMove(target.document)}><Move/>{labels.move}</button></>:null}<hr/><button className="destructive" onClick={()=>onRemove(target.type==='folder'?target.folder:target.document)}><Trash2/>{labels.remove}</button></>:null}</div>,document.body)}

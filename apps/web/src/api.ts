...
export const api={
  ...
  async tasks(workspaceId:string,projectId:string,archived=false){const result=await request<{data:Array<{id:string;number:number;projectId:string;columnId:string;title:string;description:string;kind:Task['kind'];priority:'HIGH'|'MEDIUM'|'LOW';dueDate:string|null;version:number;subtaskDone:number;subtaskTotal:number;assignees:{id:string;name:string}[];labels:string[]}>}>(`/workspaces/${workspaceId}/tasks?projectId=${projectId}&archived=${archived}`);return result.data.map((task):Task=>({id:task.id,number:task.number,projectId:task.projectId,title:task.title,description:task.description,kind:task.kind,column:task.columnId,priority:{HIGH:'高',MEDIUM:'中',LOW:'低'}[task.priority] as Task['priority'],assignee:task.assignees[0]?.name??'未分配',assigneeId:task.assignees[0]?.id??'',due:task.dueDate??'',tags:task.labels,version:task.version,subtaskDone:task.subtaskDone,subtaskTotal:task.subtaskTotal}))},
  ...
  reorderSubtasks(workspaceId:string,taskId:string,subtaskIds:string[]){return request<{ok:true}>(`/workspaces/${workspaceId}/tasks/${taskId}/subtasks/reorder`,{method:'PATCH',body:JSON.stringify({subtaskIds})})},
  ...
  setTaskGitHubLinks(workspaceId:string,taskId:string,links:Pick<GitHubReference,'kind'|'number'>[]){return request<{links:GitHubReference[]}>(`/workspaces/${workspaceId}/integrations/github/tasks/${taskId}/links`,{method:'PUT',body:JSON.stringify({links})})},

...
export const api={
  ...
  async tasks(workspaceId:string,projectId:string){const result=await request<{data:Array<{id:string;number:number;projectId:string;columnId:string;title:string;priority:'HIGH'|'MEDIUM'|'LOW';dueDate:string|null;version:number;assignees:{name:string}[]}>}>(`/workspaces/${workspaceId}/tasks?projectId=${projectId}`);return result.data.map((task):Task=>({id:task.id,number:task.number,projectId:task.projectId,title:task.title,column:task.columnId,priority:{HIGH:'高',MEDIUM:'中',LOW:'低'}[task.priority] as Task['priority'],assignee:task.assignees[0]?.name??'未分配',due:task.dueDate??'未设置',tags:[],version:task.version}))},
  createTask(workspaceId:string,task:Task){return request(`/workspaces/${workspaceId}/tasks`,{method:'POST',body:JSON.stringify({projectId:task.projectId,columnId:task.column,title:task.title,description:'',priority:{高:'HIGH',中:'MEDIUM',低:'LOW'}[task.priority],assigneeIds:[],dueDate:null,labels:task.tags})})},
  updateTask(workspaceId:string,task:Task){return request<{id:string;version:number}>(`/workspaces/${workspaceId}/tasks/${task.id}`,{method:'PATCH',body:JSON.stringify({title:task.title,columnId:task.column,priority:{高:'HIGH',中:'MEDIUM',低:'LOW'}[task.priority],version:task.version})})},
}

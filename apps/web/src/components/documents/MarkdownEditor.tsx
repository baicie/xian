  ...
  return (
    <Plate
      ...
      onValueChange={({ value: next }) =>
        onChange(editor.getApi(MarkdownPlugin).markdown.serialize({ value: next }))
      }
    >
      <Toolbar className="document-editor-toolbar">
        <MarkToolbarButton nodeType="bold" aria-label="粗体">
          <Bold />
        </MarkToolbarButton>
        <MarkToolbarButton nodeType="italic" aria-label="斜体">
          <Italic />
        </MarkToolbarButton>
        <MarkToolbarButton nodeType="underline" aria-label="下划线">
          <Underline />
        </MarkToolbarButton>

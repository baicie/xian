...
import { ColumnId, Priority, Task, TaskKind, filterTasks, moveTask } from '@/models/board'
import {
  api,
...
import { appPaths, getProjectIdFromPath, workspacePageRoutes } from './routes'
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
const copy = {
  zh: {
    ...
    deleteProject: '删除项目',
    deleteTitle: '确认删除项目？',
    deleteDescription: '项目和其中任务将被移入软删除状态，此操作不会立即擦除数据库记录。',
    delete: '删除',
    noResults: '没有匹配的任务',
    emptyColumn: '把任务拖到这里',
    addTask: '添加任务',
  ...
  en: {
    ...
    deleteTaskDescription:
      'The task will be soft deleted and hidden from the current project. Its database record is retained.',
    ...
    <ShadcnSidebar className="app-sidebar" collapsible="icon">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent side="right" align="start">
              <DropdownMenuGroup>
                ...
                {workspaces.map((item) => (
                  <DropdownMenuItem key={item.id} onClick={() => onWorkspace(item.id)}>
      ...
      <SidebarContent>
        <nav aria-label="主导航">
          ...
          <p className="nav-label nav-label--project">
            {t.project}
            <Button variant="ghost" size="icon-xs" aria-label={t.newProject} onClick={onNewProject}>
              <Plus />
            </Button>
          </p>
          {projects.map((project) => (
            <div className="project-row" key={project.id}>
              <NavLink
                className={({ isActive }) => `project-link ${isActive ? 'selected' : ''}`}
      ...
      <SidebarFooter>
        <div className="sidebar-bottom">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              render={<Button variant="ghost" className="profile" aria-label={t.account} />}
            >
            ...
            <DropdownMenuContent side="right" align="end">
              <DropdownMenuGroup>
                ...
                <DropdownMenuItem onClick={() => setLang(lang === 'zh' ? 'en' : 'zh')}>
...
function TaskDialog({
  ...
  const [confirmingDelete, setConfirmingDelete] = useState(false),
    [deletingTask, setDeletingTask] = useState(false)
  ...
  useEffect(() => {
    if (!task || task.id === 'new') {
      setGithubReferences(null)
      setGithubLinks([])
      setInitialGithubLinks([])
      return
    }
    let active = true
    Promise.all([api.githubReferences(workspaceId), api.taskGitHubLinks(workspaceId, task.id)])
      .then(([references, links]) => {
        if (active) {
          setGithubReferences(references.projectId === task.projectId ? references.items : null)
          setGithubLinks(links)
          setInitialGithubLinks(links)
        }
      })
      .catch(() => {
        if (active) {
  ...
  useEffect(() => {
    ...
    let active = true
    api
      .taskWatch(workspaceId, task.id)
      .then((value) => active && setWatching(value.watching))
      .catch(() => active && setWatching(false))
    return () => {
      active = false
    }
  }, [task, workspaceId])
  ...
  const save = async () => {
    ...
    try {
      const linkKeys = (links: GitHubReference[]) =>
        links
          .map((link) => `${link.kind}:${link.number}`)
          .sort()
          .join(',')
      if (
        draft.id !== 'new' &&
        githubReferences &&
        linkKeys(githubLinks) !== linkKeys(initialGithubLinks)
      )
        await api.setTaskGitHubLinks(
          workspaceId,
          draft.id,
          githubLinks.map(({ kind, number }) => ({ kind, number })),
        )
      await onSave(draft)
    } catch (reason) {
      toast.error(
    ...
    <Sheet open={Boolean(task)} onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="task-sheet">
            ...
            <FieldGroup className="dialog-fields">
              ...
              <FieldGroup className="form-grid">
                ...
                <Field>
                  ...
                  <ChoiceSelect
                    ...
                    options={[
                      { value: 'NONE', label: t.assignee === '负责人' ? '未分配' : 'Unassigned' },
                      ...members.map((member) => ({ value: member.id, label: member.name })),
                    ]}
                    onChange={(assigneeId) => {
                      const assignee =
                        members.find((member) => member.id === assigneeId)?.name ?? '未分配'
                      setDraft({
                        ...draft,
              ...
              {draft.id !== 'new' ? (
                <TaskWorkflow
                  ...
                  onTransition={(transition, comment) => onTransition(draft, transition, comment)}
                />
              ) : (
                <Field>
                  <FieldLabel>{t.status}</FieldLabel>
                  <Badge variant="secondary">
                    {columns.find((column) => column.id === draft.column)?.label}
                  </Badge>
                </Field>
              )}
              <Field>
              ...
              <TaskTypeFieldsEditor
                kind={draft.kind}
                value={draft.typeFields}
                en={t.taskDetails !== '任务详情'}
              ...
              {githubReferences ? (
                <Field>
                  <FieldLabel>
                    {t.taskDetails === '任务详情' ? '关联 GitHub' : 'GitHub links'}
                  </FieldLabel>
                  <div className="github-reference-list">
                    {githubReferences.length ? (
                      githubReferences.map((reference) => {
                        const checked = githubLinks.some(
                            (link) =>
                              link.kind === reference.kind && link.number === reference.number,
                          ),
                          id = `github-reference-${reference.kind}-${reference.number}`
                        return (
                          <div
                          ...
                          >
                              ...
                              onCheckedChange={() =>
                                setGithubLinks((current) =>
                                  checked
                                    ? current.filter(
                                        (link) =>
                                          link.kind !== reference.kind ||
                                          link.number !== reference.number,
                                      )
                                    : [...current, reference],
                                )
                              }
                            />
                            {reference.kind === 'PR' ? <GitPullRequest /> : <CircleDot />}
                            <label htmlFor={id}>
                              <strong>
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              aria-label={`Open ${reference.kind} ${reference.number}`}
                            >
                              <ExternalLink />
                            </a>
                          </div>
                      ...
                      <p className="github-reference-empty">
                        {t.taskDetails === '任务详情'
                          ? '仓库中暂无 Issue 或 PR'
          ...
          <SheetFooter className="task-sheet-footer">
            ...
            {draft.id !== 'new' ? (
              <Button
                ...
                onClick={() =>
                  void api
                    .setTaskWatch(workspaceId, draft.id, !watching)
                    .then((value) => setWatching(value.watching))
                }
              >
                {watching ? (
        ...
        <AlertDialog
          ...
          onOpenChange={(open) => !deletingTask && setConfirmingDelete(open)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
            ...
            <AlertDialogFooter>
              <AlertDialogCancel disabled={deletingTask}>{t.cancel}</AlertDialogCancel>
              <AlertDialogAction
                variant="destructive"
...
function CreateDialog({
  ...
  const [busy, setBusy] = useState(false),
    ...
    en = lang === 'en'
  useEffect(() => {
    if (kind === 'project') setWorkflowTemplate('DELIVERY')
  }, [kind])
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await onCreate(
        ...
        kind === 'project' ? String(values.get('code')).toUpperCase() : undefined,
        kind === 'project' ? workflowTemplate : undefined,
      )
      onClose()
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : en ? 'Creation failed' : '创建失败')
    } finally {
      setBusy(false)
    }
  }
        ...
        <form onSubmit={submit}>
          ...
          <FieldGroup className="create-form">
            <Field data-invalid={Boolean(error)}>
              <FieldLabel htmlFor="create-name">{en ? 'Name' : '名称'}</FieldLabel>
              <Input
                id="create-name"
                name="name"
                required
            ...
            {kind === 'project' ? (
              <>
                <Field>
                  <FieldLabel htmlFor="create-code">{en ? 'Code' : '项目代码'}</FieldLabel>
                  <Input
                    id="create-code"
                    name="code"
                    required
                ...
                <Field>
                  <FieldLabel>{en ? 'Workflow' : '项目流程'}</FieldLabel>
                  <ChoiceSelect
                    label={en ? 'Workflow' : '项目流程'}
                    value={workflowTemplate}
                    options={workflowTemplateOptions.map((option) => ({
                      value: option.value,
                      label: en
                        ? {
                            SIMPLE: 'Simple board',
                            DELIVERY: 'Development delivery',
                            RELEASE: 'Full development',
                          }[option.value]
                        : option.name,
                    }))}
                  ...
                  <FieldDescription>
                    {en
                      ? {
                          ...
                          DELIVERY: 'Development, testing, acceptance and return',
                          RELEASE: 'Development, testing, release and completion',
                        }[workflowTemplate]
                      : workflowTemplateOptions.find((option) => option.value === workflowTemplate)
                          ?.description}
                  </FieldDescription>
                </Field>
              </>
...
function RenameProjectDialog({
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : en ? 'Rename failed' : '重命名失败')
    } finally {
      setBusy(false)
    }
  }
...
const importRoleLabels: Record<TaskColumnRole, { zh: string; en: string }> = {
  TITLE: { zh: '任务标题', en: 'Title' },
  DESCRIPTION: { zh: '描述', en: 'Description' },
  KIND: { zh: '类型', en: 'Type' },
  PRIORITY: { zh: '优先级', en: 'Priority' },
  IGNORE: { zh: '忽略', en: 'Ignore' },
}
function mappedRole(mapping: TaskWorkbookMapping, index: number): TaskColumnRole {
  return mapping.titleColumn === index
    ? 'TITLE'
    : mapping.descriptionColumns.includes(index)
      ? 'DESCRIPTION'
      : mapping.kindColumn === index
        ? 'KIND'
        : mapping.priorityColumn === index
          ? 'PRIORITY'
          : 'IGNORE'
}
...
function TaskImportDialog({
  ...
  mapping: TaskWorkbookMapping | null
  busy: boolean
  en: boolean
  onClose: () => void
  onMappingChange: (index: number, role: TaskColumnRole) => void
  onRefresh: () => void
  onImport: () => void
}) {
  return (
    <Dialog open={Boolean(file)} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="task-import-dialog">
        <DialogHeader>
          <DialogTitle>{en ? 'Review Excel import' : '确认 Excel 导入'}</DialogTitle>
          <DialogDescription>
            {file?.name}
        ...
        {busy && !preview ? (
          <div className="import-loading">{en ? 'Analyzing workbook...' : '正在分析工作簿...'}</div>
        ) : null}
        {preview && mapping ? (
          <>
            <div className="import-summary">
              <span>
                <strong>{preview.counts.valid}</strong>
                {en ? 'Ready' : '可导入'}
              </span>
              <span>
                <strong>{preview.counts.invalid}</strong>
                {en ? 'Invalid' : '无效'}
              </span>
              <span>
                <strong>{preview.counts.duplicates}</strong>
                {en ? 'Duplicates' : '重复'}
              </span>
              <span>
                <strong>{preview.counts.ignored}</strong>
                {en ? 'Ignored' : '忽略'}
              </span>
            </div>
            <div className="import-mapping">
              <div className="import-section-head">
                <strong>{en ? 'Column mapping' : '字段映射'}</strong>
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  disabled={busy || mapping.titleColumn < 0}
                  onClick={onRefresh}
                >
                  {busy ? (en ? 'Analyzing...' : '分析中...') : en ? 'Refresh preview' : '重新分析'}
                </Button>
              </div>
              <div className="import-mapping-grid">
                {preview.columns.map((column) => (
                  <div className="import-mapping-field" key={column.index}>
                    <span>{column.header}</span>
                    <ChoiceSelect
                      label={`${column.header} ${en ? 'mapping' : '字段映射'}`}
                      value={mappedRole(mapping, column.index)}
                      options={(Object.keys(importRoleLabels) as TaskColumnRole[]).map((role) => ({
                        value: role,
                        label: importRoleLabels[role][en ? 'en' : 'zh'],
                      }))}
                      onChange={(role) => onMappingChange(column.index, role)}
            ...
            <div className="import-rows">
              <div className="import-section-head">
                <strong>{en ? 'Row report' : '行校验结果'}</strong>
                <small>
                  {en ? `${preview.counts.total} data rows` : `${preview.counts.total} 行数据`}
                </small>
              </div>
              <div className="import-table">
                <div className="import-table-head">
                  <span>{en ? 'Row' : '行'}</span>
                  <span>{en ? 'Title' : '标题'}</span>
                  <span>{en ? 'Type' : '类型'}</span>
                  <span>{en ? 'Result' : '结果'}</span>
                </div>
                {preview.rows.map((row) => (
                  <div className="import-table-row" key={row.sourceRow}>
                    ...
                    >
                      {row.errors[0] ??
                        (row.duplicate ? (en ? 'Duplicate' : '重复') : en ? 'Ready' : '可导入')}
                    </span>
                  </div>
                ))}
              </div>
        ...
        <DialogFooter>
          ...
          <Button
            type="button"
            disabled={
              busy || !preview || !mapping || mapping.titleColumn < 0 || preview.counts.valid === 0
            }
            onClick={onImport}
          >
            {busy
              ? en
                ...
                : '导入中...'
              : en
                ? `Import ${preview?.counts.valid ?? 0} tasks`
                : `导入 ${preview?.counts.valid ?? 0} 个任务`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default function App() {
  ...
  const [lang, setLang] = useState<Lang>(() => (localStorage.getItem('lang') as Lang) || 'zh'),
    [theme, setTheme] = useState<Theme>(() => (localStorage.getItem('theme') as Theme) || 'light')
  ...
  const project =
    ...
    view: 'board' | 'list' = searchParams.get('view') === 'list' ? 'list' : 'board',
    setView = (next: 'board' | 'list') =>
      setSearchParams(next === 'board' ? {} : { view: next }, { replace: true })
  const loadWorkspace = useCallback(
    async (id: string, resetPath = true) => {
      const [nextProjects, nextMembers] = await Promise.all([api.projects(id), api.members(id)])
      setWorkspaceId(id)
      ...
      setQuery('')
      if (resetPath)
        navigate(nextProjects[0] ? appPaths.project(nextProjects[0].id) : appPaths.inbox, {
          replace: true,
        })
    },
    [navigate],
  ...
  useEffect(() => {
    ...
    setError('')
    Promise.all([api.workflow(workspaceId, project.id), api.tasks(workspaceId, project.id)])
  ...
  const shownTasks = useMemo(
    () => filterTasks(tasks, query).filter((task) => kind === 'ALL' || task.kind === kind),
    [tasks, query, kind],
  )
  const selectedTasks = tasks.filter((task) => selectedTaskIds.includes(task.id)),
    selectedTaskCount = selectedTaskIds.length,
    allShownSelected =
      shownTasks.length > 0 && shownTasks.every((task) => selectedTaskIds.includes(task.id)),
    bulkTransitionTargets = commonTransitionTargets(workflowTransitions, selectedTasks)
  useEffect(() => {
    setSelectedTaskIds([])
    setConfirmingBulkDelete(false)
  }, [workspaceId, project?.id, view, query, kind])
  const reload = async () => {
  ...
  const transitionTask = async (task: Task, transition: WorkflowTransition, comment: string) => {
    ...
    const result = await api.transitionTask(
      workspaceId,
      task.id,
      transition.toColumnId,
      saved.version,
      comment,
    )
    setEditing(null)
    await reload()
    toast.success(lang === 'zh' ? result.actionName : `Task moved: ${result.actionName}`)
  }
  ...
  const toggleAllShown = (checked: boolean) =>
    setSelectedTaskIds((current) =>
      checked
        ? [...new Set([...current, ...shownTasks.map((task) => task.id)])]
        : current.filter((id) => !shownTasks.some((task) => task.id === id)),
    )
  const applyBulk = async (action: Parameters<typeof api.bulkUpdateTasks>[2], success: string) => {
    ...
    setError('')
    try {
      const result = await api.bulkUpdateTasks(workspaceId, selectedTaskIds, action)
      await reload()
      setSelectedTaskIds([])
      setConfirmingBulkDelete(false)
      toast.success(
        lang === 'zh' ? `已更新 ${result.updated} 个任务` : `${result.updated} tasks ${success}`,
      )
    } catch (reason) {
      setError(
        reason instanceof Error
  ...
  const quickCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setQuickTitle('')
      await reload()
      toast.success(lang === 'zh' ? '任务已创建' : 'Task created')
    } catch (reason) {
      setError(
        reason instanceof Error ? reason.message : lang === 'zh' ? '创建失败' : 'Creation failed',
      )
    } finally {
      setQuickBusy(false)
    }
  }
  const closeTaskImport = () => {
    setImportFile(null)
  ...
  const previewTaskImport = async (file: File, mapping?: TaskWorkbookMapping) => {
    ...
    if (!file.name.toLowerCase().endsWith('.xlsx')) {
      setError(lang === 'zh' ? '仅支持 .xlsx 文件' : 'Only .xlsx files are supported')
      return
    }
    setImportFile(file)
    setImportBusy(true)
    setError('')
    try {
      const result = await api.previewTaskImport(workspaceId, file, project.id, mapping)
      setImportPreview(result)
      setImportMapping(result.mapping)
    } catch (reason) {
      closeTaskImport()
      setError(reason instanceof Error ? reason.message : 'Excel 预览失败')
    } finally {
      setImportBusy(false)
    }
  }
  const changeImportMapping = (index: number, role: TaskColumnRole) =>
    setImportMapping((current) => {
      ...
      const next = {
        ...current,
        descriptionColumns: current.descriptionColumns.filter((value) => value !== index),
        kindColumn: current.kindColumn === index ? null : current.kindColumn,
        priorityColumn: current.priorityColumn === index ? null : current.priorityColumn,
        titleColumn: current.titleColumn === index ? -1 : current.titleColumn,
      }
      if (role === 'TITLE') next.titleColumn = index
      if (role === 'DESCRIPTION')
        next.descriptionColumns = [...next.descriptionColumns, index].sort((a, b) => a - b)
      if (role === 'KIND') next.kindColumn = index
      if (role === 'PRIORITY') next.priorityColumn = index
      return next
    })
  const confirmTaskImport = async () => {
    ...
    try {
      ...
      toast.success(
        lang === 'zh'
          ? `已导入 ${result.imported} 个任务，跳过 ${result.duplicateRows} 个重复项`
          : `Imported ${result.imported} tasks; skipped ${result.duplicateRows} duplicates`,
      )
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Excel 导入失败')
    } finally {
      setImportBusy(false)
    }
  }
  const dropImport = (event: DragEvent) => {
    event.preventDefault()
    setImportDragging(false)
    const file = event.dataTransfer.files[0]
    if (file) void previewTaskImport(file)
  }
  const move = async (id: string, column: ColumnId) => {
    const task = tasks.find((item) => item.id === id)
    if (!task || task.column === column) return
    const transition = transitionsForTask(workflowTransitions, task).find(
      (item) => item.toColumnId === column,
    )
    if (!transition) {
      setError(lang === 'zh' ? '当前状态不能流转到目标状态' : 'This transition is not allowed')
      return
    }
    if (transition.requiresComment) {
      setEditing(task)
      toast.info(lang === 'zh' ? '请在任务详情填写驳回原因' : 'Add a return reason in task details')
      return
    }
  ...
  const createProject = async (
    name: string,
    code?: string,
    workflowTemplate: WorkflowTemplateKey = 'DELIVERY',
  ) => {
  ...
  const deleteProject = async () => {
    ...
    try {
      ...
      if (deleting.id === project?.id) {
        ...
        navigate(fallback ? appPaths.project(fallback.id) : appPaths.inbox, { replace: true })
  ...
  if (auth === 'out') {
    if (inviteToken)
      return <InviteScreen token={decodeURIComponent(inviteToken)} onReady={() => void boot()} />
    if (setupToken)
      return <SetupScreen token={decodeURIComponent(setupToken)} onReady={() => void boot()} />
  ...
  if (location.pathname === appPaths.login) return <Navigate to={appPaths.home} replace />
  const sidebar = (
    <AppSidebar
    ...
    <SidebarProvider className="app-shell" style={{ '--sidebar-width': '228px' } as CSSProperties}>
      ...
      <SidebarInset className="workspace">
        ...
        <Routes>
          {project ? (
            <>
              <Route index element={<Navigate to={appPaths.project(project.id)} replace />} />
              <Route
                path={appPaths.legacyTasks.slice(1)}
                element={<Navigate to={appPaths.project(project.id)} replace />}
              />
              <Route
                path={appPaths.projectPattern.slice(1)}
                element={
                  <>
                        ...
                        <span className="title-actions">
                          <input
                            ...
                            onChange={(event) =>
                              ...
                              void previewTaskImport(event.target.files[0])
                            }
                          />
                          <Button
                            variant="outline"
                            className={`excel-drop ${importDragging ? 'is-dragging' : ''}`}
                    ...
                    <form className="quick-entry" onSubmit={quickCreate}>
                      ...
                      <Input
                        ...
                        placeholder={t.quickEntry}
                        value={quickTitle}
                        maxLength={300}
                        disabled={quickBusy}
                        onChange={(event) => setQuickTitle(event.target.value)}
                      />
                      <Button type="submit" size="sm" disabled={quickBusy || !quickTitle.trim()}>
                        {quickBusy
                          ? lang === 'zh'
                            ? '创建中…'
                            : 'Creating…'
                    ...
                    {view === 'list' && selectedTaskCount > 0 ? (
                      <section
                        className="bulk-toolbar"
                        aria-label={lang === 'zh' ? '批量操作' : 'Bulk actions'}
                      >
                        <strong>
                        ...
                        <Button
                          size="sm"
                          disabled={bulkBusy || !userId}
                          onClick={() =>
                            void applyBulk({ type: 'ASSIGN', assigneeIds: [userId] }, 'assigned')
                          }
                        >
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent>
                            <DropdownMenuGroup>
                              <DropdownMenuLabel>{t.assignee}</DropdownMenuLabel>
                              <DropdownMenuItem
                                onClick={() =>
                                  void applyBulk({ type: 'ASSIGN', assigneeIds: [] }, 'unassigned')
                                }
                              >
                                {t.unassigned}
                              </DropdownMenuItem>
                              ...
                              {members
                                ...
                                .map((member) => (
                                  <DropdownMenuItem
                                    ...
                                    onClick={() =>
                                      void applyBulk(
                                        { type: 'ASSIGN', assigneeIds: [member.id] },
                                        'assigned',
                                      )
                                    }
                                  >
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger
                            render={
                              <Button
                                ...
                                size="sm"
                                variant="outline"
                                disabled={bulkBusy || bulkTransitionTargets.length === 0}
                              />
                            }
                          >
                            {t.changeStatus}
                          ...
                          <DropdownMenuContent>
                            <DropdownMenuGroup>
                              ...
                              {columns
                                .filter((column) => bulkTransitionTargets.includes(column.id))
                                .map((column) => (
                                  <DropdownMenuItem
                                    key={column.id}
                                    onClick={() =>
                                      void applyBulk({ type: 'MOVE', columnId: column.id }, 'moved')
                                    }
                                  >
                                    <ArrowRight />
                                    {column.label}
                            ...
                            <DropdownMenuGroup>
                              ...
                              {(
                                ...
                                <DropdownMenuItem
                                  key={nextKind}
                                  onClick={() =>
                                    void applyBulk({ type: 'KIND', kind: nextKind }, 'updated')
                                  }
                                >
                                  <List />
                                  {label}
                            ...
                            <DropdownMenuGroup>
                              ...
                              {(
                                ...
                                <DropdownMenuItem
                                  key={priority}
                                  onClick={() =>
                                    void applyBulk({ type: 'PRIORITY', priority }, 'updated')
                                  }
                                >
                                  <Flag />
                                  {label}
                    ...
                    {query && shownTasks.length === 0 ? (
                      ...
                      <section className="board">
                        {columns.map((column) => {
                          const columnTasks = shownTasks.filter((task) => task.column === column.id)
                          return (
                            <div
                              className="column"
                              key={column.id}
                              onDragOver={(event) => event.preventDefault()}
                              onDrop={(event) =>
                                void move(event.dataTransfer.getData('text/plain'), column.id)
                              }
                            >
                      ...
                      <section className="list-view">
                        <div className="list-head">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => toggleAllShown(Boolean(checked))}
                          />
                          <span>{t.task}</span>
                          <span>{t.status}</span>
                        ...
                        {shownTasks.map((task) => (
                          <div
                            className="list-row"
                            data-selected={selectedTaskIds.includes(task.id) || undefined}
                            key={task.id}
                          >
                            <Checkbox
                              aria-label={`${lang === 'zh' ? '选择' : 'Select'} ${project.code}-${task.number}`}
                              checked={selectedTaskIds.includes(task.id)}
                              onCheckedChange={(checked) =>
                                toggleTaskSelection(task.id, Boolean(checked))
                              }
                            />
                            <Button variant="ghost" onClick={() => setEditing(task)}>
                              <span>
                                ...
                                {task.title}
                                {task.subtaskTotal ? (
                                  <small className="list-task-progress">
                                    <List />
                                    {task.subtaskDone}/{task.subtaskTotal}
                                  </small>
                                ) : null}
                              ...
                              <span>
                                {columns.find((column) => column.id === task.column)?.label}
                              </span>
                              <span>
                                <UserAvatar name={task.assignee} small />
                                {task.assignee}
              ...
              <Route
                ...
                element={
                  <section className="boot">
                    <Button onClick={() => setCreating('project')}>{t.createFirst}</Button>
                  </section>
                }
              />
              <Route
              ...
              element={
                ...
                >
                  <WorkspacePage
                    ...
                    onTaskDueChange={async (task, due) => {
                      try {
                        await api.updateTask(workspaceId, { ...task, due })
                        await reload()
                        toast.success(lang === 'zh' ? '截止日期已更新' : 'Due date updated')
                      } catch (reason) {
                        setError(reason instanceof Error ? reason.message : '更新截止日期失败')
                      }
                    }}
                    workspaceRole={
                      workspaces.find((item) => item.id === workspaceId)?.role ?? 'VIEWER'
                    }
                    onWorkspaceRestored={async (id) => {
                      const next = await api.workspaces()
                      setWorkspaces(next)
                      await loadWorkspace(id)
                    }}
      ...
      {project ? (
        <TaskDialog
          ...
          onDelete={async (task) => {
            ...
            setEditing(null)
            await reload()
            toast.success(lang === 'zh' ? '任务已删除' : 'Task deleted')
          }}
      ...
      <AlertDialog
        open={Boolean(contextDeleteTask)}
        onOpenChange={(open) => !deletingFromMenu && !open && setContextDeleteTask(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t.deleteTaskTitle}</AlertDialogTitle>
            <AlertDialogDescription>
              <strong>{contextDeleteTask?.title}</strong>
              <br />
              {t.deleteTaskDescription}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deletingFromMenu}>{t.cancel}</AlertDialogCancel>
            <AlertDialogAction
              variant="destructive"
              disabled={deletingFromMenu}
              onClick={async () => {
                if (!contextDeleteTask) return
                setDeletingFromMenu(true)
                try {
                  await api.deleteTask(workspaceId, contextDeleteTask.id)
                  setContextDeleteTask(null)
                  await reload()
                  toast.success(lang === 'zh' ? '任务已删除' : 'Task deleted')
                } catch (reason) {
                  setError(
            ...
            >
              {deletingFromMenu ? (lang === 'zh' ? '删除中…' : 'Deleting…') : t.deleteTask}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      <TaskImportDialog
        ...
        onRefresh={() =>
          importFile && importMapping && void previewTaskImport(importFile, importMapping)
        }
        onImport={() => void confirmTaskImport()}
      />
      <CreateDialog
        kind={creating}
        lang={lang}
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => !bulkBusy && setConfirmingBulkDelete(open)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              variant="destructive"
              disabled={bulkBusy}
              onClick={() => void applyBulk({ type: 'DELETE' }, 'deleted')}
            >
              {bulkBusy ? (lang === 'zh' ? '删除中…' : 'Deleting…') : t.bulkDelete}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      <AlertDialog open={Boolean(deleting)} onOpenChange={(open) => !open && setDeleting(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction variant="destructive" onClick={() => void deleteProject()}>
              {t.delete}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>

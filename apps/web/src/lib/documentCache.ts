...
export function readDocumentCache(workspaceId: string): CachedWorkspace | null {
  ...
  try {
    const value = JSON.parse(
      sessionStorage.getItem(key(workspaceId)) || 'null',
    ) as CachedWorkspace | null
    if (value) {
...
export function writeDocumentCache(workspaceId: string, patch: Partial<CachedWorkspace>) {
  ...
  const recent = Object.values(documents)
    .sort((a, b) => Date.parse(b.updatedAt) - Date.parse(a.updatedAt))
    .slice(0, 12)
  const next: CachedWorkspace = {
    ...current,
    ...patch,
    documents: Object.fromEntries(recent.map((item) => [item.id, item])),
    updatedAt: Date.now(),
  }
...
export function removeCachedDocument(workspaceId: string, documentId: string) {
  ...
  const next = {
    ...current,
    documents,
    selectedId: current.selectedId === documentId ? null : current.selectedId,
    updatedAt: Date.now(),
  }
  memory.set(workspaceId, next)
  try {
...
export function loadDocumentOnce(
  workspaceId: string,
  documentId: string,
  loader: () => Promise<WorkspaceDocument>,
) {
  const cacheKey = `${workspaceId}:${documentId}`,
    existing = pending.get(cacheKey)

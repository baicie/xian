import { FormEvent, lazy, ReactElement, Suspense, useEffect, useRef, useState } from 'react'
...
export default function DocumentsPage({workspaceId,projects,en}:{workspaceId:string;projects:Project[];en:boolean}) {
  ...
  const [creating,setCreating]=useState<{folderId:string|null}|null>(null),[editing,setEditing]=useState<EditAction>(null)
  ...
  const remove=async(target:DocumentFolder|DocumentSummary)=>{setBusy(true);try{const folder='parentId'in target;if(folder)await api.deleteDocumentFolder(workspaceId,target.id);else{await api.deleteDocument(workspaceId,target.id);removeCachedDocument(workspaceId,target.id);if(document?.id===target.id){setDocument(null);setDraft(null);activeId.current=null}}setRemoving(null);await refresh(true);toast.success(en?'Deleted':'已删除')}catch(reason){setError(reason instanceof Error?reason.message:(en?'Failed to delete':'删除失败'))}finally{setBusy(false)}}
  ...
  return <section className="documents-page">
    ...
    <ResizablePanelGroup className="documents-layout" orientation={isMobile?'vertical':'horizontal'}>
      <ResizablePanel defaultSize={isMobile?'180px':'270px'} minSize={isMobile?'140px':'220px'} maxSize={isMobile?'240px':'420px'}>
        <DocumentContextMenu target={{type:'root'}} labels={labels} onNewDocument={folderId=>setCreating({folderId})} onNewFolder={parentId=>setEditing({type:'folder',parentId})} onRename={target=>setEditing('parentId'in target?{type:'folder',folder:target}:{type:'rename-document',document:target})} onDuplicate={target=>void duplicate(target)} onMove={target=>setEditing({type:'move-document',document:target})} onRemove={setRemoving}>
        <aside className="document-list" aria-label={labels.title}>
          ...
          <ScrollArea className="document-tree-scroll">
            <div className="document-tree">
              {roots.map(folder=><FolderNode key={folder.id} folder={folder} folders={folders} items={items} expanded={expanded} selectedId={document?.id??null} en={en} onToggle={id=>setExpanded(current=>{const next=new Set(current);next.has(id)?next.delete(id):next.add(id);return next})} onOpen={id=>void open(id)} onPrefetch={id=>void open(id,{background:true})} labels={labels} onNewDocument={folderId=>setCreating({folderId})} onNewFolder={parentId=>setEditing({type:'folder',parentId})} onRename={target=>setEditing('parentId'in target?{type:'folder',folder:target}:{type:'rename-document',document:target})} onDuplicate={target=>void duplicate(target)} onMove={target=>setEditing({type:'move-document',document:target})} onRemove={setRemoving}/>) }
              {rootItems.map(item=><DocumentNode key={item.id} item={item} selected={item.id===document?.id} onOpen={id=>void open(id)} onPrefetch={id=>void open(id,{background:true})} labels={labels} onNewDocument={folderId=>setCreating({folderId})} onNewFolder={parentId=>setEditing({type:'folder',parentId})} onRename={target=>setEditing('parentId'in target?{type:'folder',folder:target}:{type:'rename-document',document:target})} onDuplicate={target=>void duplicate(target)} onMove={target=>setEditing({type:'move-document',document:target})} onRemove={setRemoving}/>) }
            </div>
    ...
    <AlertDialog open={Boolean(removing)} onOpenChange={open=>!busy&&!open&&setRemoving(null)}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{en?'Confirm deletion':'确认删除'}</AlertDialogTitle><AlertDialogDescription>{removing&&'parentId'in removing?(en?'Delete this folder? Its contents will move to the root.':'删除此文件夹？其中内容会移到根目录。'):(en?'Delete this document permanently?':'确定永久删除此文档？')}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={busy}>{labels.cancel}</AlertDialogCancel><AlertDialogAction variant="destructive" disabled={busy} onClick={()=>removing&&void remove(removing)}>{busy?(en?'Deleting...':'删除中...'):labels.remove}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </section>
}
...
type DocumentMenuProps={labels:Record<string,string>;onNewDocument:(folderId:string|null)=>void;onNewFolder:(parentId:string|null)=>void;onRename:(target:DocumentFolder|DocumentSummary)=>void;onDuplicate:(target:DocumentSummary)=>void;onMove:(target:DocumentSummary)=>void;onRemove:(target:DocumentFolder|DocumentSummary)=>void}

function DocumentNode({item,selected,onOpen,onPrefetch,...menuProps}:{item:DocumentSummary;selected:boolean;onOpen:(id:string)=>void;onPrefetch:(id:string)=>void}&DocumentMenuProps) {return <DocumentContextMenu target={{type:'document',document:item}} {...menuProps}><button className={`document-tree-row document-file ${selected?'active':''}`} onClick={()=>onOpen(item.id)} onMouseEnter={()=>onPrefetch(item.id)}><File/><span><strong>{item.title}</strong><small>{item.projectName||zhKind[item.kind]}</small></span><Badge variant="secondary">v{item.version}</Badge></button></DocumentContextMenu>}

function FolderNode({folder,folders,items,expanded,selectedId,en,onToggle,onOpen,onPrefetch,...menuProps}:{folder:DocumentFolder;folders:DocumentFolder[];items:DocumentSummary[];expanded:Set<string>;selectedId:string|null;en:boolean;onToggle:(id:string)=>void;onOpen:(id:string)=>void;onPrefetch:(id:string)=>void}&DocumentMenuProps) {const open=expanded.has(folder.id),children=folders.filter(item=>item.parentId===folder.id),documents=items.filter(item=>item.folderId===folder.id);return <div className="document-folder"><DocumentContextMenu target={{type:'folder',folder}} {...menuProps}><button className="document-tree-row folder-row" onClick={()=>onToggle(folder.id)}>{open?<ChevronDown/>:<ChevronRight/>}{open?<FolderOpen/>:<Folder/>}<strong>{folder.name}</strong><small>{children.length+documents.length}</small><MoreHorizontal aria-label={en?'Folder menu':'文件夹菜单'}/></button></DocumentContextMenu>{open?<div className="document-folder-children">{children.map(child=><FolderNode key={child.id} folder={child} folders={folders} items={items} expanded={expanded} selectedId={selectedId} en={en} onToggle={onToggle} onOpen={onOpen} onPrefetch={onPrefetch} {...menuProps}/>)}{documents.map(item=><DocumentNode key={item.id} item={item} selected={item.id===selectedId} onOpen={onOpen} onPrefetch={onPrefetch} {...menuProps}/>)}</div>:null}</div>}

function DocumentContextMenu({target,children,labels,onNewDocument,onNewFolder,onRename,onDuplicate,onMove,onRemove}:{target:MenuTarget;children:ReactElement}&DocumentMenuProps) {const folderId=target.type==='folder'?target.folder.id:null;return <ContextMenu><ContextMenuTrigger render={children}/><ContextMenuContent>{target.type!=='document'?<><ContextMenuItem onClick={()=>onNewDocument(folderId)}><FilePlus2/>{labels.newDocument}</ContextMenuItem><ContextMenuItem onClick={()=>onNewFolder(folderId)}><FolderPlus/>{labels.newFolder}</ContextMenuItem></>:null}{target.type!=='root'?<><ContextMenuSeparator/><ContextMenuItem onClick={()=>onRename(target.type==='folder'?target.folder:target.document)}><Pencil/>{labels.rename}</ContextMenuItem>{target.type==='document'?<><ContextMenuItem onClick={()=>onDuplicate(target.document)}><Copy/>{labels.duplicate}</ContextMenuItem><ContextMenuItem onClick={()=>onMove(target.document)}><Move/>{labels.move}</ContextMenuItem></>:null}<ContextMenuSeparator/><ContextMenuItem variant="destructive" onClick={()=>onRemove(target.type==='folder'?target.folder:target.document)}><Trash2/>{labels.remove}</ContextMenuItem></>:null}</ContextMenuContent></ContextMenu>}

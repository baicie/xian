...
describe('task type contract', () => {
  ...
  it('does not inject create defaults into partial updates', () =>
    expect(taskPatchSchema.parse({ version: 1 })).toEqual({ version: 1 }))
  it('normalizes type-specific fields without breaking old clients', () => {
    ...
    ).toMatchObject({ severity: 'CRITICAL', affectedVersion: '1.8.0' })
  })
  it('rejects unknown or oversized type-specific fields', () => {
    expect(taskSchema.safeParse({ ...base, typeFields: { unexpected: 'value' } }).success).toBe(
      false,
    )
    expect(
      taskPatchSchema.safeParse({ version: 1, typeFields: { actualResult: 'x'.repeat(10001) } })
        .success,
    ).toBe(false)
  })
  it('accepts bulk priority and type actions and rejects duplicate task ids', () => {
    const id = '33333333-3333-4333-8333-333333333333'
    expect(
      taskBulkSchema.parse({ taskIds: [id], action: { type: 'PRIORITY', priority: 'HIGH' } }).action
        .type,
    ).toBe('PRIORITY')
    expect(
      taskBulkSchema.parse({ taskIds: [id], action: { type: 'KIND', kind: 'BUG' } }).action,
    ).toEqual({ type: 'KIND', kind: 'BUG' })
    expect(
      taskBulkSchema.safeParse({ taskIds: [id, id], action: { type: 'DELETE' } }).success,
    ).toBe(false)
  })
  it('bounds explicit transition comments', () => {
    const toColumnId = '44444444-4444-4444-8444-444444444444'
    expect(taskTransitionSchema.parse({ toColumnId, version: 2 })).toEqual({
      toColumnId,
      version: 2,
      comment: '',
    })
    expect(
      taskTransitionSchema.safeParse({ toColumnId, version: 2, comment: 'x'.repeat(2001) }).success,
    ).toBe(false)
  })
})
...
describe('plan contract', () => {
  it('requires at least one bounded plan item', () => {
    const input = {
      projectId: '4b55d43e-7dd1-47a7-814c-2210f788468b',
      title: '发布计划',
      goal: '按期发布',
      items: [{ title: '完成验收' }],
    }
    expect(planCreateSchema.parse(input).items[0]).toMatchObject({
      kind: 'TASK',
      priority: 'MEDIUM',
    })

...
export const setupPasswordSchema = z
  .object({ token: z.string().min(16), password: z.string().min(10).max(128) })
  .strict()
...
export const projectSchema = z
  .object({
    ...
    color: z
      ...
      .regex(/^#[0-9a-fA-F]{6}$/)
      .default('#2367d1'),
    workflowTemplate: z.enum(['SIMPLE', 'DELIVERY', 'RELEASE']).default('DELIVERY'),
  })
  .strict()
export const taskTypeFieldsSchema = z
  .object({
    workContent: z.string().max(20000).default(''),
    completionCriteria: z.string().max(10000).default(''),
    userStory: z.string().max(10000).default(''),
    background: z.string().max(20000).default(''),
    acceptanceCriteria: z.string().max(10000).default(''),
    businessValue: z.string().max(10000).default(''),
    reproductionSteps: z.string().max(10000).default(''),
    expectedResult: z.string().max(10000).default(''),
    actualResult: z.string().max(10000).default(''),
    environment: z.string().max(2000).default(''),
    severity: z.enum(['BLOCKER', 'CRITICAL', 'MAJOR', 'MINOR']).default('MAJOR'),
    affectedVersion: z.string().max(100).default(''),
  })
  .strict()
...
export const taskPatchSchema = z
  .object({
    ...
    typeFields: taskTypeFieldsSchema.optional(),
...
export const taskBulkSchema = z
  .object({
    taskIds: bulkTaskIds,
    action: z.discriminatedUnion('type', [
      z
        .object({ type: z.literal('ASSIGN'), assigneeIds: z.array(z.string().uuid()).max(20) })
        .strict(),
      z.object({ type: z.literal('MOVE'), columnId: z.string().uuid() }).strict(),
      z.object({ type: z.literal('KIND'), kind: z.enum(['TASK', 'STORY', 'BUG']) }).strict(),
      z
        .object({ type: z.literal('PRIORITY'), priority: z.enum(['HIGH', 'MEDIUM', 'LOW']) })
        .strict(),
...
export const documentUpdateSchema = documentCreateSchema
  ...
  .strict()
export const documentFolderCreateSchema = z
  .object({

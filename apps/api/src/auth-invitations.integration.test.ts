...
describe('registration controls and invitations', () => {
  ...
  beforeAll(async () => {
    ...
    for (const name of (await readdir(migrationDirectory)).filter((item) => item.endsWith('.sql')).sort()) {
      await migrationClient.begin(async (sql) => sql.unsafe(await readFile(new URL(name, migrationDirectory), 'utf8')))
    }
  ...
  it('allows exactly the first account to bootstrap a closed instance', async () => {
    ...
    const registrations = await Promise.all(candidates.map((email, index) => request(app.getHttpServer()).post('/api/v1/auth/register').send({ email, password, name: `Owner ${index + 1}`, workspaceName: `Phase 2 ${index + 1}` })))
    expect(registrations.map((response) => response.status).sort()).toEqual([201, 403])
    expect(registrations.find((response) => response.status === 403)?.body.code).toBe('REGISTRATION_CLOSED')
    ...
    const workspaces = await request(app.getHttpServer()).get('/api/v1/workspaces').set('Cookie', ownerCookie).expect(200)
  ...
  it('creates, lists, revokes, and audits an invitation', async () => {
    ...
    const listed = await request(app.getHttpServer()).get(`/api/v1/workspaces/${workspaceId}/invitations`).set('Cookie', ownerCookie).expect(200)
    ...
    const afterRevoke = await request(app.getHttpServer()).get(`/api/v1/workspaces/${workspaceId}/invitations`).set('Cookie', ownerCookie).expect(200)
    expect(afterRevoke.body.find((item: { id: string }) => item.id === invitation.id)).toMatchObject({ status: 'REVOKED' })

    const audit = await request(app.getHttpServer()).get(`/api/v1/workspaces/${workspaceId}/audit-logs`).set('Cookie', ownerCookie).expect(200)
    expect(audit.body.map((item: { action: string }) => item.action)).toEqual(expect.arrayContaining(['invitation.created', 'invitation.revoked']))
    expect(audit.body.find((item: { action: string }) => item.action === 'invitation.created')).toMatchObject({ actorName: expect.any(String), actorEmail: expect.stringContaining('@'), afterData: { email, role: 'MEMBER' } })

    const concurrentEmail = `concurrent-invite-${runId}@example.com`
    await Promise.all([0, 1].map(() => request(app.getHttpServer())
      .post(`/api/v1/workspaces/${workspaceId}/members`)
      .set('Cookie', ownerCookie)
      .set('x-csrf-token', ownerCsrf)
      .send({ email: concurrentEmail, role: 'VIEWER' })
      .expect(201)))
    const concurrentInvites = await request(app.getHttpServer()).get(`/api/v1/workspaces/${workspaceId}/invitations`).set('Cookie', ownerCookie).expect(200)
    expect(concurrentInvites.body.filter((item: { email: string; status: string }) => item.email === concurrentEmail && item.status === 'PENDING')).toHaveLength(1)
  })
  ...
  it('lets an existing account accept an invitation exactly once', async () => {
    ...
    await request(app.getHttpServer()).post('/api/v1/auth/register').send({ email, password, name: 'Existing', workspaceName: 'Existing workspace' }).expect(201)
    const login = await request(app.getHttpServer()).post('/api/v1/auth/login').send({ email, password }).expect(201)
    ...
    const attempts = await Promise.all([
      request(app.getHttpServer()).post('/api/v1/auth/accept-invite').set('Cookie', cookie).set('x-csrf-token', csrf).send({ token }),
      request(app.getHttpServer()).post('/api/v1/auth/accept-invite').set('Cookie', cookie).set('x-csrf-token', csrf).send({ token }),
    ])
  ...
  it('provisions an account and consumes its setup link exactly once', async () => {
    ...
    const provisions = await Promise.all([0, 1].map((index) => request(app.getHttpServer())
    ...
    const loginBeforeSetup = await request(app.getHttpServer()).post('/api/v1/auth/login').send({ email, password }).expect(401)
    expect(loginBeforeSetup.body.code).toBe('SETUP_REQUIRED')
    const preview = await request(app.getHttpServer()).get(`/api/v1/auth/setup/${token}`).expect(200)
    ...
    expect(attempts.find((response) => response.status === 201)?.headers['set-cookie'][0]).toContain('session=')

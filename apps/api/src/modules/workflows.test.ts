...
describe('resolveTransition',()=>{
  const transitions=[{fromColumnId:'backlog',toColumnId:'active',name:'开始开发',bugName:'开始修复',requiresComment:false},{fromColumnId:'review',toColumnId:'active',name:'驳回修改',bugName:'验证失败',requiresComment:true}]

  it('rejects a move that skips the configured workflow',()=>{
    expect(()=>resolveTransition(transitions,'backlog','done','TASK','')).toThrowError(expect.objectContaining({code:'WORKFLOW_TRANSITION_NOT_ALLOWED'}))
  })

  it('requires a reason for rejection and chooses bug-specific language',()=>{
    expect(()=>resolveTransition(transitions,'review','active','BUG','  ')).toThrowError(expect.objectContaining({code:'WORKFLOW_COMMENT_REQUIRED'}))
    expect(resolveTransition(transitions,'review','active','BUG',' 回归失败 ')).toMatchObject({actionName:'验证失败',comment:'回归失败'})
  })
})

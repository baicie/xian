...
const configInput=z.object({repoUrl:z.string().url(),token:z.string().min(20).max(500),projectId:z.string().uuid(),syncTasks:z.boolean().default(true),syncDocuments:z.boolean().default(true),pullIssues:z.boolean().default(true)}).strict()
const repoResponse=z.object({full_name:z.string()})
const diagnosticRepoResponse=z.object({full_name:z.string(),private:z.boolean().default(false),permissions:z.object({pull:z.boolean().optional(),push:z.boolean().optional(),admin:z.boolean().optional(),maintain:z.boolean().optional(),triage:z.boolean().optional()}).optional()})
...
type Integration={workspaceId:string;projectId:string;owner:string;repo:string;tokenCiphertext:string;tokenIv:string;tokenTag:string;syncTasks:boolean;syncDocuments:boolean;pullIssues:boolean}

@Injectable()
export class GitHubService {
  constructor(private readonly db:DatabaseService,private readonly workspaces:WorkspaceService){}

  async get(workspaceId:string,userId:string){await this.workspaces.role(workspaceId,userId,'workspace.manage');const [row]=await this.db.client`SELECT project_id AS "projectId",owner,repo,token_last4 AS "tokenLast4",sync_tasks AS "syncTasks",sync_documents AS "syncDocuments",pull_issues AS "pullIssues",updated_at AS "updatedAt" FROM github_integrations WHERE workspace_id=${workspaceId}`;return row??null}
  async configure(workspaceId:string,userId:string,raw:unknown){await this.workspaces.role(workspaceId,userId,'workspace.manage');const input=parse(configInput,raw),repo=parseGitHubRepo(input.repoUrl),[project]=await this.db.client`SELECT id FROM projects WHERE id=${input.projectId} AND workspace_id=${workspaceId} AND deleted_at IS NULL`;if(!project)throw new NotFoundException({code:'PROJECT_NOT_FOUND',message:'项目不存在'});await this.github(input.token,repo.owner,repo.repo,'',{},repoResponse);const encrypted=encryptSecret(input.token);await this.db.client`INSERT INTO github_integrations(workspace_id,project_id,owner,repo,token_ciphertext,token_iv,token_tag,token_last4,created_by,sync_tasks,sync_documents,pull_issues) VALUES(${workspaceId},${input.projectId},${repo.owner},${repo.repo},${encrypted.ciphertext},${encrypted.iv},${encrypted.tag},${input.token.slice(-4)},${userId},${input.syncTasks},${input.syncDocuments},${input.pullIssues}) ON CONFLICT(workspace_id) DO UPDATE SET project_id=excluded.project_id,owner=excluded.owner,repo=excluded.repo,token_ciphertext=excluded.token_ciphertext,token_iv=excluded.token_iv,token_tag=excluded.token_tag,token_last4=excluded.token_last4,sync_tasks=excluded.sync_tasks,sync_documents=excluded.sync_documents,pull_issues=excluded.pull_issues,updated_at=now()`;return this.get(workspaceId,userId)}
  ...
  async push(workspaceId:string,userId:string){await this.workspaces.role(workspaceId,userId,'workspace.manage');const config=await this.config(workspaceId),token=this.token(config);let tasks=0,documents=0
    const taskRows=config.syncTasks?await this.db.client<Record<string,any>[]>`SELECT t.id,t.title,t.description,t.kind,t.priority,t.due_date AS "dueDate",c.name AS "columnName",c.state_type='DONE' AS completed FROM tasks t JOIN board_columns c ON c.id=t.column_id WHERE t.workspace_id=${workspaceId} AND t.project_id=${config.projectId} AND t.deleted_at IS NULL`:[]
    ...
    const docs=config.syncDocuments?await this.db.client<Record<string,any>[]>`SELECT id,title,content FROM documents WHERE workspace_id=${workspaceId} AND (project_id=${config.projectId} OR project_id IS NULL) AND status<>'ARCHIVED'`:[]
  ...
  async pull(workspaceId:string,userId:string){await this.workspaces.role(workspaceId,userId,'workspace.manage');const config=await this.config(workspaceId);if(!config.pullIssues)return{imported:0,conflicts:0,disabled:true};const token=this.token(config),issues=await this.githubPages(token,config.owner,config.repo,'/issues?state=all&per_page=100'),[column]=await this.db.client<{id:string}[]>`SELECT id FROM board_columns WHERE project_id=${config.projectId} AND workspace_id=${workspaceId} ORDER BY position LIMIT 1`;let imported=0,conflicts=0
  ...
  async references(workspaceId:string,userId:string){await this.workspaces.role(workspaceId,userId,'task.read');const config=await this.config(workspaceId),items=await this.githubPages(this.token(config),config.owner,config.repo,'/issues?state=all&per_page=100');return{projectId:config.projectId,items:items.map(item=>toGitHubReference(item,config.owner,config.repo))}}
  async diagnostics(workspaceId:string,userId:string){await this.workspaces.role(workspaceId,userId,'workspace.manage');const config=await this.config(workspaceId),token=this.token(config),repository=await this.github(token,config.owner,config.repo,'',{},diagnosticRepoResponse),issues=await this.github(token,config.owner,config.repo,'/issues?state=all&per_page=1',{},issuesResponse),permissions=repository.permissions;return{ok:true,repository:repository.full_name,private:repository.private,issuesReadable:Array.isArray(issues),repositoryWritable:Boolean(permissions?.push||permissions?.admin||permissions?.maintain),checkedAt:new Date().toISOString()}}
  ...
  private async config(workspaceId:string){const [config]=await this.db.client<Integration[]>`SELECT workspace_id AS "workspaceId",project_id AS "projectId",owner,repo,token_ciphertext AS "tokenCiphertext",token_iv AS "tokenIv",token_tag AS "tokenTag",sync_tasks AS "syncTasks",sync_documents AS "syncDocuments",pull_issues AS "pullIssues" FROM github_integrations WHERE workspace_id=${workspaceId}`;if(!config)throw new NotFoundException({code:'GITHUB_NOT_CONNECTED',message:'尚未连接 GitHub 仓库'});return config}
  ...
  private async githubPages(token:string,owner:string,repo:string,path:string){const items:z.infer<typeof issue>[]=[];for(let page=1;page<=10;page++){const batch=await this.github(token,owner,repo,`${path}${path.includes('?')?'&':'?'}page=${page}`,{},issuesResponse);items.push(...batch);if(batch.length<100)break}return items}
...
export class GitHubController {
  ...
  @Get('diagnostics')diagnostics(@Req() req:AppRequest,@Param('workspaceId') workspaceId:string){return this.github.diagnostics(workspaceId,req.user!.id)}

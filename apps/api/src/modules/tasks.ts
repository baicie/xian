...
export class TaskService {
  ...
  async create(workspaceId:string,userId:string,input:ReturnType<typeof taskSchema.parse>) {
    ...
    return this.db.client.begin(async sql=>{
      ...
      const [task]=await sql<{id:string;version:number}[]>`INSERT INTO tasks(workspace_id,project_id,column_id,number,title,description,kind,type_fields,priority,creator_id,due_date,position) VALUES(${workspaceId},${input.projectId},${input.columnId},${project.number},${input.title},${input.description},${input.kind},${JSON.stringify(input.typeFields)}::jsonb,${input.priority},${userId},${input.dueDate},${positionRows[0]?.position??1000}) RETURNING id,version`
    ...
    return this.db.client.begin(async sql=>{
      ...
      for(const taskId of input.taskIds){
        ...
        if(input.action.type==='ASSIGN'){
          ...
          for(const assigneeId of input.action.assigneeIds){await sql`INSERT INTO task_assignees(workspace_id,task_id,user_id) VALUES(${workspaceId},${taskId},${assigneeId})`;if(assigneeId!==userId&&!previous.some(item=>item.userId===assigneeId))await sql`INSERT INTO notifications(workspace_id,user_id,task_id,actor_id,title,body,action) VALUES(${workspaceId},${assigneeId},${taskId},${userId},${`已指派任务：${task.title}`},'你成为了该任务的负责人','task.assigned')`}
...
export class TaskController {
  ...
  @Get(':id/watch') async watchState(@Req() req:AppRequest,@Param('workspaceId') workspaceId:string,@Param('id') id:string){await this.workspaces.role(workspaceId,req.user!.id,'task.read');const [row]=await this.db.client`SELECT 1 FROM task_watchers tw JOIN tasks t ON t.id=tw.task_id WHERE tw.workspace_id=${workspaceId} AND tw.task_id=${id} AND tw.user_id=${req.user!.id} AND t.deleted_at IS NULL`;return{watching:Boolean(row)}}
  @Put(':id/watch') async watch(@Req() req:AppRequest,@Param('workspaceId') workspaceId:string,@Param('id') id:string,@Body() body:unknown){await this.workspaces.role(workspaceId,req.user!.id,'task.read');const input=parse(z.object({watching:z.boolean()}).strict(),body);if(input.watching){const [row]=await this.db.client`INSERT INTO task_watchers(workspace_id,task_id,user_id) SELECT ${workspaceId},${id},${req.user!.id} WHERE EXISTS(SELECT 1 FROM tasks WHERE id=${id} AND workspace_id=${workspaceId} AND deleted_at IS NULL) ON CONFLICT DO NOTHING RETURNING task_id`;if(!row){const [existing]=await this.db.client`SELECT 1 FROM task_watchers WHERE task_id=${id} AND user_id=${req.user!.id}`;if(!existing)throw new NotFoundException({code:'TASK_NOT_FOUND',message:'任务不存在'})}}else await this.db.client`DELETE FROM task_watchers WHERE workspace_id=${workspaceId} AND task_id=${id} AND user_id=${req.user!.id}`;return{watching:input.watching}}

...
export class WorkflowTransitionError extends Error {
  constructor(
    readonly code: 'WORKFLOW_TRANSITION_NOT_ALLOWED' | 'WORKFLOW_COMMENT_REQUIRED',
    message: string,
  ) {
    super(message)
  }
...
export function resolveTransition(
  ...
  const transition = transitions.find(
    (item) => item.fromColumnId === fromColumnId && item.toColumnId === toColumnId,
  )
  if (!transition)
  ...
  if (transition.requiresComment && !normalizedComment)
    throw new WorkflowTransitionError('WORKFLOW_COMMENT_REQUIRED', '该状态流转必须填写原因')
  return {
    ...transition,
    actionName: kind === 'BUG' ? transition.bugName : transition.name,
...
export function workflowTemplate(key: WorkflowTemplateKey) {
  return workflowTemplates.find((template) => template.key === key)!
}
...
export async function installWorkflow(
  ...
  const template = workflowTemplate(key),
    columnIds = new Map<string, string>()
  for (const [index, column] of template.columns.entries()) {
    const [created] = await sql<
      { id: string }[]
    >`INSERT INTO board_columns(workspace_id,project_id,key,name,color,state_type,position) VALUES(${workspaceId},${projectId},${column.key},${column.name},${column.color},${column.type},${(index + 1) * 1000}) RETURNING id`

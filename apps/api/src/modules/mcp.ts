...
type HealthTask={kind:string;dueDate:string|null;columnName:string;assigneeCount:number;completed:boolean}
export function summarizeProjectHealth(tasks:HealthTask[],today=new Date().toISOString().slice(0,10)){
  const active=tasks.filter(task=>!task.completed)
  const byStatus=Object.fromEntries(Array.from(new Set(tasks.map(task=>task.columnName))).map(status=>[status,tasks.filter(task=>task.columnName===status).length]))
  return{total:tasks.length,active:active.length,completed:tasks.length-active.length,openBugs:active.filter(task=>task.kind==='BUG').length,overdue:active.filter(task=>task.dueDate&&task.dueDate<today).length,unassigned:active.filter(task=>task.assigneeCount===0).length,byStatus}
}
...
export class McpService {
  ...
  server(identity:McpIdentity){
    ...
    server.registerTool('list_projects',{description:'List projects in the authorized 闲序 workspace.',inputSchema:{},annotations:{readOnlyHint:true,openWorldHint:false}},async()=>run('list_projects',()=>this.projects.list(identity.workspaceId,identity.userId)))
    ...
    server.registerTool('summarize_project_health',{description:'Summarize project delivery health using task status, bugs, overdue dates, and assignment gaps. This is read-only.',inputSchema:{projectId:z.string().uuid()},annotations:{readOnlyHint:true,openWorldHint:false}},async({projectId})=>run('summarize_project_health',async()=>{await this.projects.list(identity.workspaceId,identity.userId);const rows=await this.db.client<HealthTask[]>`SELECT t.kind,t.due_date::text AS "dueDate",c.name AS "columnName",(SELECT count(*)::int FROM task_assignees ta WHERE ta.task_id=t.id) AS "assigneeCount",c.state_type='DONE' AS completed FROM tasks t JOIN board_columns c ON c.id=t.column_id WHERE t.workspace_id=${identity.workspaceId} AND t.project_id=${projectId} AND t.deleted_at IS NULL AND t.archived_at IS NULL`;const [project]=await this.db.client`SELECT id,name,code FROM projects WHERE id=${projectId} AND workspace_id=${identity.workspaceId} AND deleted_at IS NULL`;if(!project)throw new Error('Project not found');return{project,...summarizeProjectHealth(rows)}}))
    server.registerTool('draft_weekly_update',{description:'Draft a Markdown weekly update from recent project activity and current delivery health. This is read-only and does not save a document.',inputSchema:{projectId:z.string().uuid(),days:z.number().int().min(1).max(31).default(7)},annotations:{readOnlyHint:true,openWorldHint:false}},async({projectId,days})=>run('draft_weekly_update',async()=>{const [project]=await this.db.client<{name:string;code:string}[]>`SELECT name,code FROM projects WHERE id=${projectId} AND workspace_id=${identity.workspaceId} AND deleted_at IS NULL`;if(!project)throw new Error('Project not found');const rows=await this.db.client<HealthTask[]>`SELECT t.kind,t.due_date::text AS "dueDate",c.name AS "columnName",(SELECT count(*)::int FROM task_assignees ta WHERE ta.task_id=t.id) AS "assigneeCount",c.state_type='DONE' AS completed FROM tasks t JOIN board_columns c ON c.id=t.column_id WHERE t.workspace_id=${identity.workspaceId} AND t.project_id=${projectId} AND t.deleted_at IS NULL AND t.archived_at IS NULL`,health=summarizeProjectHealth(rows),activities=await this.db.client<{action:string;title:string;actor:string;createdAt:string}[]>`SELECT a.action,t.title,u.name AS actor,a.created_at AS "createdAt" FROM activities a JOIN tasks t ON t.id=a.task_id JOIN users u ON u.id=a.actor_id WHERE a.workspace_id=${identity.workspaceId} AND t.project_id=${projectId} AND a.created_at>=now()-(${days}::text||' days')::interval ORDER BY a.created_at DESC LIMIT 50`;const markdown=[`# ${project.name} 周报`,``, `## 当前状态`,`- 活跃任务：${health.active}` ,`- 已完成：${health.completed}`,`- 未关闭 Bug：${health.openBugs}`,`- 已逾期：${health.overdue}`,`- 未分配：${health.unassigned}`,``,`## 本周动态`,...(activities.length?activities.map(item=>`- ${item.actor} · ${item.title} · ${item.action}`):['- 暂无活动记录']),``,`> 此内容由项目数据生成，发布前请人工核对。`].join('\n');return{project,days,health,markdown}}))
    server.registerTool('preview_plan_application',{description:'Preview the tasks a plan would create and whether it was already applied. This never creates or updates tasks.',inputSchema:{planId:z.string().uuid()},annotations:{readOnlyHint:true,openWorldHint:false}},async({planId})=>run('preview_plan_application',async()=>{const plan=await this.plans.get(identity.workspaceId,identity.userId,planId) as {id:string;title:string;goal:string;status:string;items:{title:string;kind:string;priority:string;taskId:string|null}[]};return{planId:plan.id,title:plan.title,goal:plan.goal,status:plan.status,alreadyApplied:plan.status==='APPLIED',taskCount:plan.items.length,items:plan.items.map(item=>({title:item.title,kind:item.kind,priority:item.priority,existingTaskId:item.taskId}))}}))
    ...
    server.registerPrompt('plan_project_work',{description:'Plan project work as a reviewable 闲序 draft.',argsSchema:{projectId:z.string().uuid(),goal:z.string().min(1)}},async({projectId,goal})=>({messages:[{role:'user',content:{type:'text',text:`Inspect project ${projectId} and its tasks, then propose a focused plan for: ${goal}. Save it with create_plan_draft. Do not call apply_plan until a human explicitly approves the draft.`}}]}))
    server.registerPrompt('review_project_risks',{description:'Review delivery risks without changing project data.',argsSchema:{projectId:z.string().uuid()}},async({projectId})=>({messages:[{role:'user',content:{type:'text',text:`Call summarize_project_health for project ${projectId}. Explain the most important delivery risks, cite the returned counts, and suggest mitigations. Do not create or update any project data.`}}]}))
    server.registerPrompt('prepare_weekly_update',{description:'Prepare a project weekly update for human review.',argsSchema:{projectId:z.string().uuid(),days:z.string().optional()}},async({projectId,days})=>({messages:[{role:'user',content:{type:'text',text:`Call draft_weekly_update for project ${projectId} covering ${days??'7'} days. Refine the wording without inventing facts. Return the Markdown for human review; do not save or publish it.`}}]}))

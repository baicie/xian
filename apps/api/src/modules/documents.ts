...
export class DocumentService {
  ...
  async get(workspaceId: string, userId: string, documentId: string) {
    ...
    const [document] = await this.db
      .client`SELECT id,project_id AS "projectId",folder_id AS "folderId",title,kind,status,content,version,created_at AS "createdAt",updated_at AS "updatedAt" FROM documents WHERE id=${documentId} AND workspace_id=${workspaceId}`
    if (!document)
  ...
  async update(workspaceId: string, userId: string, documentId: string, input: UpdateDocument) {
    ...
    return this.db.client.begin(async (sql) => {
      const [current] = await sql<
        { version: number }[]
      >`SELECT version FROM documents WHERE id=${documentId} AND workspace_id=${workspaceId} FOR UPDATE`
      if (!current)
        throw new NotFoundException({ code: 'DOCUMENT_NOT_FOUND', message: '文档不存在' })
      if (current.version !== input.version)
        throw new ConflictException({
          code: 'DOCUMENT_VERSION_CONFLICT',
          message: '文档已被其他人更新，请刷新后重试',
        })
      const [document] =
  ...
  async versions(workspaceId: string, userId: string, documentId: string) {
    ...
    return this.db
      .client`SELECT v.id,v.version,v.title,v.status,v.change_note AS "changeNote",v.created_at AS "createdAt",u.name AS "createdByName" FROM document_versions v JOIN users u ON u.id=v.created_by WHERE v.workspace_id=${workspaceId} AND v.document_id=${documentId} ORDER BY v.version DESC`
  }

  async folders(workspaceId: string, userId: string) {
    await this.workspaces.role(workspaceId, userId, 'document.read')
    return this.db
      .client`SELECT id,parent_id AS "parentId",name,created_at AS "createdAt",updated_at AS "updatedAt" FROM document_folders WHERE workspace_id=${workspaceId} ORDER BY name`
  }
  ...
  async createFolder(workspaceId: string, userId: string, input: CreateFolder) {
    ...
    const [folder] = await this.db
      .client`INSERT INTO document_folders(workspace_id,parent_id,name,created_by) VALUES(${workspaceId},${input.parentId},${input.name},${userId}) RETURNING id,parent_id AS "parentId",name,created_at AS "createdAt",updated_at AS "updatedAt"`
    return folder
  }
  ...
  async deleteFolder(workspaceId: string, userId: string, folderId: string) {
    ...
    await this.assertFolder(workspaceId, folderId)
    await this.db
      .client`DELETE FROM document_folders WHERE id=${folderId} AND workspace_id=${workspaceId}`
    return { ok: true }
  }
  ...
  async delete(workspaceId: string, userId: string, documentId: string) {
    ...
    const result = await this.db
      .client`DELETE FROM documents WHERE id=${documentId} AND workspace_id=${workspaceId} RETURNING id`
    if (!result.length)
      throw new NotFoundException({ code: 'DOCUMENT_NOT_FOUND', message: '文档不存在' })
    return { ok: true }
  }
  ...
  private async assertFolder(workspaceId: string, folderId: string) {
    const [folder] = await this.db
      .client`SELECT id FROM document_folders WHERE id=${folderId} AND workspace_id=${workspaceId}`
    if (!folder)
      throw new NotFoundException({ code: 'DOCUMENT_FOLDER_NOT_FOUND', message: '文件夹不存在' })
  }

  private async descendantFolderIds(workspaceId: string, folderId: string) {
    const rows = await this.db.client<
      { id: string }[]
    >`WITH RECURSIVE descendants AS (SELECT id FROM document_folders WHERE id=${folderId} AND workspace_id=${workspaceId} UNION ALL SELECT f.id FROM document_folders f JOIN descendants d ON f.parent_id=d.id WHERE f.workspace_id=${workspaceId}) SELECT id FROM descendants`
...
export class DocumentController {
  ...
  @Get() list(@Req() req: AppRequest, @Param('workspaceId') workspaceId: string) {
    return this.documents.list(workspaceId, req.user!.id)
  }
  @Post() create(
    @Req() req: AppRequest,
    ...
    return this.documents.create(workspaceId, req.user!.id, parse(documentCreateSchema, body))
  }
  @Get('folders') folders(@Req() req: AppRequest, @Param('workspaceId') workspaceId: string) {
    return this.documents.folders(workspaceId, req.user!.id)
  }
  @Post('folders') createFolder(
    ...
    return this.documents.createFolder(
      workspaceId,
      req.user!.id,
      parse(documentFolderCreateSchema, body),
    )
  }
  @Patch('folders/:folderId') updateFolder(
    @Req() req: AppRequest,
    @Param('workspaceId') workspaceId: string,
    @Param('folderId') folderId: string,
    @Body() body: unknown,
  ) {
    return this.documents.updateFolder(
      workspaceId,
      req.user!.id,
      folderId,
      parse(documentFolderUpdateSchema, body),
    )
  }
  @Delete('folders/:folderId') deleteFolder(
    ...
    return this.documents.deleteFolder(workspaceId, req.user!.id, folderId)
  }
  @Get(':documentId') get(
    @Req() req: AppRequest,
    @Param('workspaceId') workspaceId: string,
    ...
    return this.documents.get(workspaceId, req.user!.id, documentId)
  }
  @Patch(':documentId') update(
    @Req() req: AppRequest,
    @Param('workspaceId') workspaceId: string,
    @Param('documentId') documentId: string,
    @Body() body: unknown,
    ...
    return this.documents.update(
      ...
      req.user!.id,
      documentId,
      parse(documentUpdateSchema, body),
    )
  }
  @Post(':documentId/duplicate') duplicate(
    @Req() req: AppRequest,
    @Param('workspaceId') workspaceId: string,
    @Param('documentId') documentId: string,
  ) {
    return this.documents.duplicate(workspaceId, req.user!.id, documentId)
  }
  @Delete(':documentId') delete(
    ...
    return this.documents.delete(workspaceId, req.user!.id, documentId)
  }
  @Get(':documentId/versions') versions(
    @Req() req: AppRequest,
    @Param('workspaceId') workspaceId: string,
    @Param('documentId') documentId: string,
  ) {
    return this.documents.versions(workspaceId, req.user!.id, documentId)
  }
}

...
export class TransferService {
  ...
  async snapshot(workspaceId:string,userId:string):Promise<WorkspaceSnapshot>{
    ...
    const [members,projects,columns,tasks,assignees,labels,checks,comments,documents,versions,plans,planItems,assets]=await Promise.all([
    ...
    ]) as [ExportRow[],ExportRow[],ExportRow[],ExportRow[],ExportRow[],ExportRow[],ExportRow[],ExportRow[],ExportRow[],ExportRow[],ExportRow[],ExportRow[],ExportRow[]]
  ...
  async restore(data:Uint8Array,userId:string){
    ...
    try{return await this.db.client.begin(async sql=>{
      ...
      const projectIds=new Map<string,string>(),columnIds=new Map<string,string>(),taskIds=new Map<string,string>(),assetIds=new Map<string,string>()
      ...
      for(const project of snapshot.projects){
        ...
        for(const task of project.tasks){const [next]=await sql<{id:string}[]>`INSERT INTO tasks(workspace_id,project_id,column_id,number,title,description,kind,type_fields,priority,creator_id,due_date,position,version,archived_at) VALUES(${workspace!.id},${created!.id},${columnIds.get(task.columnSourceId)!},${task.number},${task.title},${task.description},${task.kind},${JSON.stringify(task.typeFields)}::jsonb,${task.priority},${userId},${task.dueDate},${task.position},${task.version},${task.archived?now:null}) RETURNING id`;taskIds.set(task.sourceId,next!.id)

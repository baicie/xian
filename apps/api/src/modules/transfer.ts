...
export class TransferService {
  ...
  async snapshot(workspaceId:string,userId:string):Promise<WorkspaceSnapshot>{
    ...
    const [members,projects,columns,tasks,assignees,labels,checks,comments,documents,versions,plans,planItems]=await Promise.all([
      ...
      this.db.client`SELECT id AS "sourceId",name,code,description,color,archived_at IS NOT NULL AS archived FROM projects WHERE workspace_id=${workspaceId} AND deleted_at IS NULL ORDER BY created_at`,
      this.db.client`SELECT id AS "sourceId",project_id AS "projectSourceId",name,color,position FROM board_columns WHERE workspace_id=${workspaceId} ORDER BY position`,
      this.db.client`SELECT id AS "sourceId",project_id AS "projectSourceId",column_id AS "columnSourceId",number,title,description,kind,priority,due_date AS "dueDate",position,version,archived_at IS NOT NULL AS archived FROM tasks WHERE workspace_id=${workspaceId} AND deleted_at IS NULL ORDER BY project_id,number`,
      ...
      this.db.client`SELECT c.task_id AS "taskSourceId",c.body,u.email AS "authorEmail",c.created_at AS "createdAt" FROM comments c JOIN users u ON u.id=c.author_id WHERE c.workspace_id=${workspaceId} AND c.deleted_at IS NULL ORDER BY c.created_at`,
  ...
  async restore(data:Uint8Array,userId:string){
    ...
    return this.db.client.begin(async sql=>{
      ...
      const projectIds=new Map<string,string>(),columnIds=new Map<string,string>(),taskIds=new Map<string,string>()
      for(const project of snapshot.projects){
        const [created]=await sql<{id:string}[]>`INSERT INTO projects(workspace_id,name,code,description,color,lead_id,archived_at) VALUES(${workspace!.id},${project.name},${project.code},${project.description},${project.color},${userId},${project.archived?now:null}) RETURNING id`;projectIds.set(project.sourceId,created!.id)
        for(const column of project.columns){const [next]=await sql<{id:string}[]>`INSERT INTO board_columns(workspace_id,project_id,name,color,position) VALUES(${workspace!.id},${created!.id},${column.name},${column.color},${column.position}) RETURNING id`;columnIds.set(column.sourceId,next!.id)}
        for(const task of project.tasks){const [next]=await sql<{id:string}[]>`INSERT INTO tasks(workspace_id,project_id,column_id,number,title,description,kind,priority,creator_id,due_date,position,version,archived_at) VALUES(${workspace!.id},${created!.id},${columnIds.get(task.columnSourceId)!},${task.number},${task.title},${task.description},${task.kind},${task.priority},${userId},${task.dueDate},${task.position},${task.version},${task.archived?now:null}) RETURNING id`;taskIds.set(task.sourceId,next!.id)
          ...
          for(const comment of task.comments){const [author]=await sql<{id:string}[]>`SELECT id FROM users WHERE email=${comment.authorEmail}`;await sql`INSERT INTO comments(workspace_id,task_id,author_id,body,created_at) VALUES(${workspace!.id},${next!.id},${author?.id??userId},${comment.body},${comment.createdAt})`}
        }
...
const upload=FileInterceptor('file',{limits:{fileSize:25*1024*1024,files:1}})
